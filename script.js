...
// ------------------- Sprawdzenie DOM i przypisanie elementów -------------------
  ...
  loginForm.addEventListener("submit", e => {
    ...
    auth.signInWithEmailAndPassword(email, password)
      ...
      .catch(err => {
        ...
        if (errorMsg) errorMsg.textContent = "Błąd: " + err.message;
      });
  });
}

// ------------------- Wylogowanie (app.html) -------------------
if (logoutBtn) {
...
// ------------------- Ochrona i inicjalizacja (app.html) -------------------
if (testForm) {
...
function initApp() {
  ...
  // ------------------- CRUD -------------------
  window.editTestCase = function(idx) {
    testCases = getAllTestCases();
  ...
  window.deleteTestCase = function(idx) {
  ...
  window.setCritical = function(idx) {
    testCases = getAllTestCases();
    let tc = testCases[idx];
  ...
  window.showHistory = function(idx) {
    testCases = getAllTestCases();
    let tc = testCases[idx];
  ...
  function renderTable() {
    ...
    data.forEach((tc, idx) => {
      ...
      tr.innerHTML = `
        ...
        <td>
          ${tc.status === 'Pass' ? '<span class="badge pass-badge">Pass</span>' : ''}
          ${tc.status === 'Fail' ? '<span class="badge fail-badge">Fail</span>' : ''}
